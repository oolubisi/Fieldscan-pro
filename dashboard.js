...
export function renderProjects() {
  ...
  container.innerHTML = filtered.map(p => `<div class="card" data-project-id="${escapeAttr(p.projectId)}" onclick="window.loadProjectConsoleHub('${escapeAttr(p.projectId)}')" style="border-left:5px solid #2d7a49; cursor:pointer; display:flex; gap:12px; align-items:center;"><img src="${escapeAttr(getDirectImageUrl(p.avatar || ''))}" onerror="this.src='${PLACEHOLDER_AVATAR}'" style="width:50px; height:50px; border-radius:6px; object-fit:cover;"><div style="flex:1;"><strong>${escapeHtml(p.projectId)}</strong><br><small style="color:var(--muted);">${escapeHtml(p.clientName)}</small></div></div>`).join('');
}
...
export function renderVendors() {
  ...
  container.innerHTML = filtered.map(v => `<div class="card" data-vendor-id="${escapeAttr(v.vendorId)}" onclick="window.openVendorById(this.dataset.vendorId)" style="display:flex; gap:12px; align-items:center; cursor:pointer;"><img src="${escapeAttr(getDirectImageUrl(v.passport || ''))}" onerror="this.src='${PLACEHOLDER_AVATAR}'" style="width:50px; height:50px; border-radius:6px; object-fit:cover;"><div style="flex:1;"><strong>${escapeHtml(v.company)}</strong><br><small style="color:var(--muted);">${escapeHtml(v.trade)} • ${escapeHtml(v.phone1)}</small></div></div>`).join('');
}
